...
async function awardRandomBadgesToDemoUsers() {
  ...
  demoUsers.forEach(snap => {
    const user = snap.data();
    if (user.role === "demo") {
      eligibleUsers.push({ uid: snap.id, ...user });
    }
  });

  const lowLevelTasks = rewardTasks.filter(task =>
    ["first-post",  "viewed-by-1",  "viewed-1", "avatar-uploaded", "collab-request-1", "referral-5", "feedback-1", "login-1", "commentMade-1"].includes(task.id)
  );

  for (const user of eligibleUsers) {
    const numBadges = Math.floor(Math.random() * 3) + 3; // 3 to 5 badges
    const selected = [...lowLevelTasks].sort(() => 0.5 - Math.random()).slice(0, numBadges);

    const updates = {
      badges: user.badges || {},
      rewardsCompleted: [...(user.rewardsCompleted || [])],
      points: user.points || 0
    };

    selected.forEach(task => {
      if (!updates.rewardsCompleted.includes(task.id)) {
        const daysAgo = Math.floor(Math.random() * 30) + 1;
        const earnedDate = Timestamp.fromDate(new Date(Date.now() - daysAgo * 24 * 60 * 60 * 1000));
        const badgeType = task.type;

        updates.rewardsCompleted.push(task.id);
        updates.points += task.reward.points;

        updates.badges[badgeType] = updates.badges[badgeType] || { levels: [] };
        updates.badges[badgeType].levels = [...new Set([...updates.badges[badgeType].levels, 1])];
        updates.badges[badgeType].lastEarned = earnedDate;
      }
    });

    await updateDoc(doc(db, "users", user.uid), updates);
    console.log(`🎉 Awarded ${selected.length} demo badges to ${user.uid}`);
  }
}

 awardRandomBadgesToDemoUsers();